...
public class ClockArrayEditorPanel extends JPanel {
	...
	private ClockArrayListModel model = new ClockArrayListModel();
	...
	public ClockArrayEditorPanel(PropertyEditorSupport clockEditor) {
		editor = clockEditor;
	...
	public void changeValue() {		
		...
		//editor.setValue(array); //TEGO NIE MA W DOUBLE ARRAY EDITOR
	...
	public void setArray(int i, Clock value) {
		if (0 <= i && i < array.length) {
			Clock copy = new Clock();//tak ma byc????????
			copy.hours = value.hours;
			copy.minutes = value.minutes;
			copy.seconds = value.seconds;
			
			model.setValue(i, copy);
...
class ClockArrayListModel extends AbstractListModel<Clock> {
